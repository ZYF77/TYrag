...
from enterprise.gateway.feed_audit_middleware import _request_caller, _request_username
...
def test_request_username_only_reads_safe_auth_state():
    assert _request_username({"audit_username": "zkadmin"}) == "zkadmin"
    assert _request_username({"audit_username": "  eam-user  "}) == "eam-user"
    assert _request_username({"audit_username": ""}) == ""
    assert _request_username({"authorization": "Bearer secret-token"}) == ""
...
async def test_inquiry_also_lands_in_http_event_ring(isolated_gateway_db, monkeypatch, tmp_path):
    ...
    assert events[0]["caller_username"] == "biz-user-001"
