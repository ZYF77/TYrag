...
class TestGatewayConfig:
    ...
    def test_attachment_vision_defaults_and_override(self, monkeypatch):
        monkeypatch.delenv("ENTERPRISE_ATTACHMENT_VISION_ENABLED", raising=False)
        monkeypatch.delenv("ENTERPRISE_ATTACHMENT_VISION_LLM_ID", raising=False)
        monkeypatch.delenv("ENTERPRISE_ATTACHMENT_VISION_TIMEOUT_SECONDS", raising=False)
        cfg = GatewayConfig()
        assert cfg.attachment_vision_enabled is True
        assert cfg.attachment_vision_llm_id == ""
        assert cfg.attachment_vision_timeout_seconds == 30.0
        ...
        monkeypatch.setenv("ENTERPRISE_ATTACHMENT_VISION_TIMEOUT_SECONDS", "15")
        overridden = GatewayConfig()
        assert overridden.attachment_vision_enabled is False
        assert overridden.attachment_vision_llm_id == "vl-ocr@local"
        assert overridden.attachment_vision_timeout_seconds == 15.0
