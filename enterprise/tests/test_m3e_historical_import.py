...
@asynccontextmanager
async def _historical_import_db():
...
async def _success_processor(db: AsyncConnection, calls: list[str], failures: int = 0):
...
async def _review_failure_processor(db: AsyncConnection, calls: list[str]):
    async def process(event):
        calls.append(event.event_id)
        payload = json.loads(event.payload)
        doc = await get_mapping(
...
async def test_batch_contract_checkpoint_and_idempotent_replay():
    async with _historical_import_db() as db:
        calls: list[str] = []
